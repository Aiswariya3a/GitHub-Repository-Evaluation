...
def resolve_rubric_snapshot(repo_row=None, session_record=None):
    ...
    if rubric_config:
        return {
            "name": rubric_config.get("name") or (session_record.get("rubric_name") if session_record else None) or "C Programming - Banking Transaction Assignment",
            "version": rubric_config.get("version") or (session_record.get("rubric_version") if session_record else None) or 1,
            "type": rubric_config.get("rubric_type") or (session_record.get("rubric_type") if session_record else None) or "System",
    ...
    return {
        "name": (session_record.get("rubric_name") if session_record else None) or "C Programming - Banking Transaction Assignment",
        "version": (session_record.get("rubric_version") if session_record else None) or 1,
        "type": (session_record.get("rubric_type") if session_record else None) or "System",
        "config": None,
        "max_score": 80,
        "category_max_scores": {},
        "question_titles": {},
...
def get_plagiarism(roll, plag_df):
    if plag_df.empty:
        return plag_df
...
def build_question_table(question_data, body_style):
...
def generate_preamble(output_dir, all_rolls, eval_map, repo_map, plag_df,
                      title_style, subtitle_style, section_style, body_style,
                      report_rubric, rubric_max_score):
    """Generate consolidated report with summary statistics and question-wise breakdown."""
    doc = SimpleDocTemplate(os.path.join(output_dir, "preamble.pdf"), pagesize=A4, rightMargin=30, leftMargin=30, topMargin=28, bottomMargin=28)
    content = []
...
def merge_pdfs(output_dir):
    merger = PdfMerger()
    merger.append(os.path.join(output_dir, "preamble.pdf"))

    student_reports_dir = os.path.join(output_dir, "student_reports")
    for pdf in sorted(os.listdir(student_reports_dir)):
        if pdf.lower().endswith(".pdf"):
            merger.append(os.path.join(student_reports_dir, pdf))

    merger.write(os.path.join(output_dir, "Final_Consolidated_Report.pdf"))
    merger.close()
...
def generate_pdf(session_id: str, output_dir: str) -> str:
    ...
    saved_repositories = [repo for repo in store.list_repositories(session_id) if repo["status"] == "Completed"]

    if not saved_repositories:
        raise ValueError("No completed evaluations to report.")

    report_rubric = resolve_rubric_snapshot(saved_repositories[0] if saved_repositories else None, session_record)
    ...
    os.makedirs(os.path.join(output_dir, "student_reports"), exist_ok=True)
    ...
    for roll in all_rolls:
        ...
        rubric_snapshot = resolve_rubric_snapshot(repo, session_record)
        ...
        doc = SimpleDocTemplate(
            os.path.join(output_dir, "student_reports", f"{roll}.pdf"),
            pagesize=A4,
            rightMargin=30,
            leftMargin=30,
            topMargin=28,
        ...
        if questions:
            for question_id in sorted(questions.keys()):
                ...
                content.append(build_question_table(question_data, body_style))
                content.append(Spacer(1, 8))
        else:
        ...
        plag_matches = get_plagiarism(roll, plag_df)
    ...
    # Generate preamble
    generate_preamble(output_dir, all_rolls, eval_map, repo_map, plag_df,
                      title_style, subtitle_style, section_style, body_style,
                      report_rubric, rubric_max_score)
    ...
    result_path = os.path.join(output_dir, "Final_Consolidated_Report.pdf")
    return result_path
