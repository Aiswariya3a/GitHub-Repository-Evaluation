...
class GitHubService:
    ...
    def get_repo_metadata(self, repo_url: str) -> dict:
        """Lightweight fetch of repo metadata via /repos/{path} (no cloning, no pagination).

        Returns a dict with description, language, topics, stars, forks, etc.
        ...
        try:
            ...
            resp = requests.get(f"https://api.github.com/repos/{repo_path}", headers=self.headers)
    ...
    def get_commits(self, repo_url: str, max_pages: int = 2) -> list[dict]:
        repo_path = self.clean_url(repo_url).split("github.com/")[1]
        commits = []
        url = f"https://api.github.com/repos/{repo_path}/commits?per_page=50"
        pages = 0
        try:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(files, list):
                    has_readme = any(isinstance(item, dict) and "readme" in item.get("name", "").lower() for item in files)
        except Exception:
            pass
        return {
